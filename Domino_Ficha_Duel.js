...
var Domino_BolaGeomDesktop = null;
var Domino_BolaGeomMobile = null;
...
var Domino_Ficha = function() {
    ...
    this.AnguloNoCero = function(A) {
        return (Math.abs(A) > this.EPS_ANG);
    };
...
//        this.Ficha.name = "Ficha";
        ...
        var esMovil = (typeof(Domino) !== "undefined" && Domino && Domino.EsMovilVisual === true);
        if (esMovil) {
            if (Domino_BolaGeomMobile === null) Domino_BolaGeomMobile = new THREE.SphereGeometry(0.1, 12, 12);
        }
        else {
            if (Domino_BolaGeomDesktop === null) Domino_BolaGeomDesktop = new THREE.SphereGeometry(0.1, 24, 24);
        }
        this.Bola = new THREE.Mesh(esMovil ? Domino_BolaGeomMobile : Domino_BolaGeomDesktop, new THREE.MeshPhongMaterial( { color: 0x999999, specular : 0xccffcc, transparent : false, opacity: 1.0 } ));
            ...
            switch (FichaOrigen.Direccion) {
                case "centro" :
                    ...
                    if (FichaOrigen === Domino.Partida.FichaIzquierda && FichaOrigen === Domino.Partida.FichaDerecha) {
                        if (RamaForzada === "izquierda") {
                            Ret = this.BuscarPosIzq(FichaOrigen);
                            this.Direccion = "izquierda";
                            this.Rama = "izquierda";
                        }
                        else if (RamaForzada === "derecha") {
...
//                Domino.Partida.FichaDerecha = this;
        ...
        // Mode reprise/synchronisation: placer immédiatement sans animation.
        if (Instantaneo === true) {
            this.Ficha.scale.set(1.0, 1.0, 1.0);
            this.Escala = 1.0;
            this.Ficha.position.set(Ret.PosX, this.Ficha.position.y, Ret.PosZ);
            this.Ficha.rotation.z = Ret.RotZ;
            this.Ficha.rotation.x = -Math.PI / 2;
            ...
            if (Domino.Partida && typeof(Domino.Partida.AvanzarTurnoDespuesColocar) === "function") {
                Domino.Partida.AvanzarTurnoDespuesColocar(this);
            }
            else {
    ...
    this.ValorLibre = function() {
        switch (this.Direccion) {
            ...
            case "izquierda"    : return (this.AnguloEq(this.Ficha.rotation.z, Math.PI)) ? this.Valores[1] :  this.Valores[0];
            case "derecha"      : return (this.AnguloEq(this.Ficha.rotation.z, Math.PI)) ? this.Valores[0] :  this.Valores[1];
            case "arriba"       : return (this.AnguloEq(this.Ficha.rotation.z, Math.PI / 2)) ? this.Valores[1] : this.Valores[0];
            case "abajo"        : return (this.AnguloEq(this.Ficha.rotation.z, Math.PI / 2)) ? this.Valores[0] : this.Valores[1];
        }
        return -1;
    };
    ...
    this.BuscarPosIzq = function(FichaDesde) {
        ...
        if (this.FichaDoble() === true) { 
            ...
            Ret.PosX = (FichaDesde.FichaDoble() === true && this.AnguloNoCero(FichaDesde.Ficha.rotation.z)) ? FichaDesde.Ficha.position.x - 1.5 : FichaDesde.Ficha.position.x - 2.0;            
        }
        return Ret;
    };
